...
import (
	...
	"github.com/juancruzrobledo/jr-stack/internal/tui/screens"
	"github.com/juancruzrobledo/jr-stack/internal/update"
	"github.com/juancruzrobledo/jr-stack/internal/update/upgrade"
)
...
type Model struct {
	...
	// UninstallResult holds the list of items cleaned up by the Gentleman uninstall.
	UninstallResult []string

	// UninstallDone is true once the uninstall operation has completed.
	UninstallDone bool
...
func (m Model) View() string {
	...
	case ScreenUninstallResult:
		return screens.RenderUninstallResult(m.UninstallResult)
	default:
		return ""
	}
}
	...
	if m.Screen == ScreenClaudeModelPicker {
		...
		if handled {
			...
			if updated != nil {
				...
				} else if m.shouldShowOpenCodeModelPickerScreen() {
...
func (m Model) confirmSelection() (tea.Model, tea.Cmd) {
	...
	case ScreenWelcome:
		...
		case 6:
			...
			result, _ := cli.UninstallGentleman(homeDir)
	...
	case ScreenPreset:
		...
		if m.Cursor < len(options) {
			...
			if m.shouldShowOpenCodeModelPickerScreen() {
				cachePath := opencode.DefaultCachePath()
				if _, err := osStatModelCache(cachePath); err == nil {
					m.ModelPicker = screens.NewModelPickerState(cachePath)
					m.Selection.ModelAssignments = nil
	...
	case ScreenModelPicker:
		...
		if len(m.ModelPicker.AvailableIDs) == 0 {
			...
			// No model providers detected — go back.
			if m.shouldShowClaudeModelPickerScreen() {
				m.setScreen(ScreenClaudeModelPicker)
			} else {
				m.setScreen(ScreenPreset)
		...
		// Back from ModelPicker: go to ClaudeModelPicker if Claude selected, else Preset.
		if m.ModelConfigMode {
			m.ModelConfigMode = false
			m.setScreen(ScreenModelConfig)
			return m, nil
...
func (m Model) goBack() Model {
	...
	//   - OpenCode flow: ModelPicker (if OpenCode + cache)
...
func (m Model) shouldShowOpenCodeModelPickerScreen() bool {
