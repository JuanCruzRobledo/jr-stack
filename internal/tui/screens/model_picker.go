...
type ModelPickerState struct {
	...
	SDDModels    map[string][]opencode.Model // provider ID -> OPSX-capable models
...
func renderPhaseList(
	...
	b.WriteString(styles.TitleStyle.Render("Assign Models to OPSX Actions"))
	b.WriteString("\n\n")

	if len(state.AvailableIDs) == 0 {
		b.WriteString(styles.WarningStyle.Render("OpenCode has not been run yet — model cache not found."))
		b.WriteString("\n")
		b.WriteString(styles.SubtextStyle.Render("Run 'opencode' once, then re-run 'jr-stack sync' to assign models."))
		b.WriteString("\n")
		b.WriteString(styles.SubtextStyle.Render("Using default model assignments for now."))
		b.WriteString("\n\n")
		b.WriteString(renderOptions([]string{"← Back to OPSX mode"}, cursor))
		b.WriteString("\n")
