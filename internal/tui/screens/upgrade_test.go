...
func TestRenderUpgrade_AllUpToDate(t *testing.T) {
	results := []update.UpdateResult{
		{
			Tool:             update.ToolInfo{Name: "jr-stack"},
...
func TestRenderUpgrade_UpdatesAvailable(t *testing.T) {
	...
	if !strings.Contains(out, "jr-stack") {
		t.Errorf("RenderUpgrade(updates available) should contain tool name 'jr-stack'; got:\n%s", out)
	}
