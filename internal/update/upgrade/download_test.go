...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"jr-stack_1.5.0_darwin_arm64/jr-stack", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "jr-stack")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "jr-stack", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}
