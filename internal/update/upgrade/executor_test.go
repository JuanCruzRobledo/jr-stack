...
import (
	...
	"github.com/juancruzrobledo/jr-stack/internal/update"
)
...
func TestExecute_NoopWhenNothingIsExecutable(t *testing.T) {
	results := []update.UpdateResult{
		makeResult("jr-stack", update.UpToDate, "1.0.0", "1.0.0", update.InstallBinary),
...
func TestExecute_DevBuildOnlyNoBackupCreated(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("jr-stack", update.DevBuild, "dev", "1.0.0", update.InstallBinary),
	}
...
// jr-stack with DevBuild status must appear in Results as UpgradeSkipped
...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	// jr-stack (DevBuild) MUST appear as UpgradeSkipped with a ManualHint.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "jr-stack" {
			r := report.Results[i]
			devResult = &r
		}
	}
	if devResult == nil {
		t.Fatalf("jr-stack (DevBuild) must appear in Results — was not found")
	}
	if devResult.Status != UpgradeSkipped {
		t.Errorf("jr-stack DevBuild Status = %q, want UpgradeSkipped", devResult.Status)
	}
	if devResult.ManualHint == "" {
		t.Errorf("jr-stack DevBuild ManualHint must be non-empty")
	}
...
func TestExecute_InstallNotInvoked(t *testing.T) {
	...
	//   - github.com/juancruzrobledo/jr-stack/internal/cli
	//
...
// A DevBuild tool (e.g. jr-stack with version="dev") MUST appear in UpgradeReport.Results
...
func TestExecute_DevBuildSurfacedAsSkipped(t *testing.T) {
	...
	// jr-stack (DevBuild) MUST appear in results as UpgradeSkipped.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "jr-stack" {
			r := report.Results[i]
	...
	if devResult == nil {
		t.Fatalf("jr-stack DevBuild must appear in Results as UpgradeSkipped, but was not found")
	}
	...
	if devResult.ManualHint == "" {
		t.Errorf("jr-stack DevBuild ManualHint must be non-empty — should explain dev/source build")
	}
...
func TestExecute_ManualFallbackSurfacedAsSkippedNotFailed(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("jr-stack", update.UpdateAvailable, "1.0.0", "1.5.0", update.InstallBinary),
	}
	results[0].UpdateHint = "See https://github.com/Gentleman-Programming/jr-stack/releases"
...
func TestExecute_UpgradeBackupManifestHasUpgradeMetadata(t *testing.T) {
	...
	backupRoot := filepath.Join(homeDir, ".jr-stack", "backups")
