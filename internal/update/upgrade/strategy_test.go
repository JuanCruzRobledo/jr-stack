...
// self-replace for jr-stack is NOT attempted in Phase 1 — it must return a
...
func TestRunStrategy_BinaryWindowsSelfUpdateSkipped(t *testing.T) {
	...
	r := update.UpdateResult{
		...
		LatestVersion: "1.5.0",
		ReleaseURL:    "https://github.com/Gentleman-Programming/jr-stack/releases/tag/v1.5.0",
	}
...
func TestManualFallbackHint(t *testing.T) {
	r := update.UpdateResult{
		...
		LatestVersion: "1.5.0",
		UpdateHint:    "See https://github.com/Gentleman-Programming/jr-stack/releases",
	}
	profile := system.PlatformProfile{OS: "windows", PackageManager: "winget"}
...
func TestBrewUpgrade_RunsUpdateBeforeUpgrade(t *testing.T) {
	...
	err := brewUpgrade(context.Background(), "jr-stack")
	if err != nil {
...
func TestBrewUpgrade_UpdateFailureIsNonFatal(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		return exec.Command("echo", "Upgraded jr-stack")
	}
