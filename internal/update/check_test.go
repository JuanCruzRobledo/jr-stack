...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "jr-stack uses build var",
			tool:         ToolInfo{Name: "jr-stack", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "jr-stack dev build",
			tool:         ToolInfo{Name: "jr-stack", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "jr-stack-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "jr-stack-update-check")
	}
}
...
func TestCheckAll(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case contains(path, "jr-stack"):
			release = githubRelease{TagName: "v1.5.0", HTMLURL: "https://github.com/Gentleman-Programming/jr-stack/releases/tag/v1.5.0"}
	...
	// jr-stack: 1.5.0 local == 1.5.0 remote → UpToDate
	assertResult(t, results[0], "jr-stack", UpToDate, "1.5.0", "1.5.0")
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// jr-stack has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("jr-stack status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("jr-stack expected error, got nil")
	}
...
func TestUpdateHint(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "jr-stack macOS",
			tool:    ToolInfo{Name: "jr-stack"},
			profile: system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			want:    "brew upgrade jr-stack",
		},
		{
			name:    "jr-stack linux",
			tool:    ToolInfo{Name: "jr-stack"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "curl -fsSL https://raw.githubusercontent.com/Gentleman-Programming/jr-stack/main/scripts/install.sh | bash",
		},
		{
			name:    "jr-stack windows",
			tool:    ToolInfo{Name: "jr-stack"},
			profile: system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			want:    "irm https://raw.githubusercontent.com/Gentleman-Programming/jr-stack/main/scripts/install.ps1 | iex",
		},
		{
		...
		},
		{
			...
			tool:    ToolInfo{Name: "engram"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "jr-stack upgrade (downloads pre-built binary)",
		},
		{
			name:    "engram windows",
			tool:    ToolInfo{Name: "engram"},
...
func TestRegistryContents(t *testing.T) {
	...
	expected := map[string]struct {
		...
		"jr-stack": {owner: "JuanCruzRobledo", repo: "jr-stack"},
	...
	// jr-stack must have nil DetectCmd.
	if Tools[0].DetectCmd != nil {
		t.Fatalf("jr-stack DetectCmd should be nil")
	}
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (jr-stack) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just jr-stack to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("jr-stack dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes jr-stack to be reported as DevBuild.
...
// when the running jr-stack binary reports version "dev", it is identified as a
// DevBuild and NOT reported as UpdateAvailable or VersionUnknown.
//
// The spec says:
//   - Dev build MUST be reported as development-build semantic
//   - jr-stack self-upgrade is skipped while engram/gga remain eligible
func TestCheckFiltered_DevBuildSemanticsForGentleAI(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return exec.Command("false") }
	Tools = []ToolInfo{Tools[0]} // jr-stack only
	...
	r := results[0]
	if r.Tool.Name != "jr-stack" {
		t.Fatalf("tool = %q, want jr-stack", r.Tool.Name)
	}
...
// jr-stack with "dev" version gets DevBuild while engram with a real version stays eligible.
func TestCheckFiltered_DevBuildSkipNotEligible(t *testing.T) {
	...
	// Only jr-stack and engram for this test
	...
	// jr-stack should be DevBuild
	if results[0].Status != DevBuild {
		t.Fatalf("jr-stack status = %q, want DevBuild", results[0].Status)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	// Only engram and gga for this test (skip jr-stack to avoid dev-build behavior)
