...
func TestInjectWritesSkillFilesForClaude(t *testing.T) {
	...
	// Only non-OpenSpec skills are written by the skills component; OpenSpec skills are
...
func TestInjectSkipsOpenSpecSkills(t *testing.T) {
	home := t.TempDir()

	// OpenSpec skills should be silently skipped — they are installed by the SDD component.
	...
	// Only the non-OpenSpec skill (skill-creator) should be written.
	if len(result.Files) != 1 {
		t.Fatalf("Inject() files len = %d, want 1 (only skill-creator)", len(result.Files))
	}

	// OpenSpec skill files must not be created by the skills component.
	for _, id := range []model.SkillID{model.SkillOpenSpecInit, model.SkillOpenSpecApply} {
		path := filepath.Join(home, ".claude", "skills", string(id), "SKILL.md")
		if _, statErr := os.Stat(path); statErr == nil {
			t.Fatalf("skills component must not write OpenSpec skill %q — it belongs to the SDD component", id)
		}
	}
}
