...
func Inject(homeDir string, adapter agents.Adapter) (InjectionResult, error) {
	...
	case model.StrategySeparateMCPFiles:
		...
		// `engram setup <agent>` is invoked. jr-stack's Inject() runs after
		...
		// See: https://github.com/Gentleman-Programming/jr-stack/issues (engram absolute path regression)
...
// jr-stack runs Inject() after setup, so we must not overwrite that absolute
