...
// written to a markdown file outside of <!-- jr-stack: --> markers.
//
...
// <!-- jr-stack: --> marker). A fingerprint that exists only inside a marker
...
// written by the standalone ATL installer before jr-stack superseded it.
...
// <!-- jr-stack:sdd-orchestrator --> section, so keeping both wastes ~150
...
// Legacy marker constants from the original Gentleman gentle-ai installation.
// These are used to detect and strip old content that would conflict with
// the new jr-stack markers.
const (
	legacyMarkerPrefix = "<!-- gentle-ai:"
	legacyClosePrefix  = "<!-- /gentle-ai:"
)

// StripLegacyGentleAISections removes ALL sections wrapped in the old
// <!-- gentle-ai:SECTION_ID --> markers from a markdown file.
// This prevents conflicts when jr-stack injects its own <!-- jr-stack:* --> sections.
// Safe to call on any file: returns content unchanged if no legacy markers are found.
func StripLegacyGentleAISections(content string) string {
	for {
		openIdx := strings.Index(content, legacyMarkerPrefix)
		if openIdx < 0 {
			break
		}

		// Find the section ID from the open marker.
		afterPrefix := content[openIdx+len(legacyMarkerPrefix):]
		endOfMarker := strings.Index(afterPrefix, markerSuffix)
		if endOfMarker < 0 {
			break
		}
		sectionID := afterPrefix[:endOfMarker]
		closeMarkerStr := legacyClosePrefix + sectionID + markerSuffix

		closeIdx := strings.Index(content[openIdx:], closeMarkerStr)
		if closeIdx < 0 {
			// No matching close marker — remove just the open marker line.
		...
		// Remove the entire section including both markers.
		absClose := openIdx + closeIdx
		after := content[absClose+len(closeMarkerStr):]
...
//
// Before injecting, any legacy <!-- gentle-ai:SECTION_ID --> markers for the SAME
// section are automatically stripped to prevent conflicting instructions.
func InjectMarkdownSection(existing, sectionID, content string) string {
	// Strip the legacy gentle-ai marker for this specific section if present.
	legacyOpen := legacyMarkerPrefix + sectionID + markerSuffix
	legacyClose := legacyClosePrefix + sectionID + markerSuffix
	if strings.Contains(existing, legacyOpen) {
		openIdx := strings.Index(existing, legacyOpen)
		closeIdx := strings.Index(existing, legacyClose)
		if openIdx >= 0 && closeIdx >= 0 && closeIdx > openIdx {
			after := existing[closeIdx+len(legacyClose):]
			if len(after) > 0 && after[0] == '\n' {
				after = after[1:]
			}
