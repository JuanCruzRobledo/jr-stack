...
import (
	...
	"github.com/juancruzrobledo/jr-stack/internal/components/filemerge"
...
type InjectOptions struct {
	...
	// <!-- jr-stack:strict-tdd-mode --> marker section is injected into
...
//
// Walking upward means users can run jr-stack from any subdirectory of their
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	// findProjectRoot walks upward from WorkspaceDir so jr-stack can be
...
func injectFileAppend(homeDir string, adapter agents.Adapter) (InjectionResult, error) {
	...
	// Strip legacy Gentleman gentle-ai markers (<!-- gentle-ai:* -->) so they
	// don't conflict with our new <!-- jr-stack:* --> markers.
	existing = filemerge.StripLegacyGentleAISections(existing)
...
func hasLegacyBareOrchestrator(content string) bool {
	markedIdx := strings.Index(content, "<!-- jr-stack:sdd-orchestrator -->")
	if markedIdx >= 0 {
...
//   - end at the next managed marker ("<!-- jr-stack:") if present, else EOF
...
func stripBareOrchestratorForFilePrompt(content string) string {
	if markedIdx := strings.Index(content, "<!-- jr-stack:sdd-orchestrator -->"); markedIdx >= 0 {
		prefix := content[:markedIdx]
	...
	if rel := strings.Index(content[start:], "<!-- jr-stack:"); rel >= 0 {
		end = start + rel
	}
...
func injectMarkdownSections(homeDir string, adapter agents.Adapter, assignments map[string]model.ClaudeModelAlias) (InjectionResult, error) {
	...
	if hasSDDOrchestrator(existing) && !strings.Contains(existing, "<!-- jr-stack:sdd-orchestrator -->") {
...
func injectClaudeModelAssignments(content string, assignments map[string]model.ClaudeModelAlias) (string, error) {
	const openMarker = "<!-- jr-stack:sdd-model-assignments -->"
	const closeMarker = "<!-- /jr-stack:sdd-model-assignments -->"
