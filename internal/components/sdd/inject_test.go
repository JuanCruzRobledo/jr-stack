...
import (
	...
	windsurfagent "github.com/juancruzrobledo/jr-stack/internal/agents/windsurf"
	"github.com/juancruzrobledo/jr-stack/internal/assets"
...
func TestInjectClaudeCustomModelAssignments(t *testing.T) {
	...
	if !strings.Contains(text, "<!-- jr-stack:sdd-model-assignments -->") {
...
func TestInjectCursorWritesSDDOrchestratorAndSkills(t *testing.T) {
	...
	promptPath := filepath.Join(home, ".cursor", "rules", "jr-stack.mdc")
...
<!-- END:agent-teams-lite -->`
	...
	sddSection := "<!-- jr-stack:sdd-orchestrator -->\nYou are a COORDINATOR.\n<!-- /jr-stack:sdd-orchestrator -->\n"
...
func TestStripBareOrchestratorSection_DoesNotStripIfMarkersPresent(t *testing.T) {
	...
	// the `!strings.Contains(existing, "<!-- jr-stack:sdd-orchestrator -->")` guard
...
// the <!-- jr-stack:strict-tdd-mode --> marker with its content.
func TestInjectStrictTDDEnabledInjectsMarkerIntoClaude(t *testing.T) {
	...
	text := string(content)
	if !strings.Contains(text, "<!-- jr-stack:strict-tdd-mode -->") {
		t.Fatal("CLAUDE.md missing <!-- jr-stack:strict-tdd-mode --> open marker")
	}
	if !strings.Contains(text, "<!-- /jr-stack:strict-tdd-mode -->") {
		t.Fatal("CLAUDE.md missing <!-- /jr-stack:strict-tdd-mode --> close marker")
	}
...
func TestInjectClaudeDoesNotStripMarkedSection(t *testing.T) {
	...
	if !strings.Contains(string(after1), "<!-- jr-stack:sdd-orchestrator -->") {
...
func TestInjectWindsurf_WorkflowsCopiedToWorkspace(t *testing.T) {
	...
	// Verify opsx-propose.md was written to .windsurf/workflows/
	workflowPath := filepath.Join(workspace, ".windsurf", "workflows", "opsx-propose.md")
...
func TestInjectWindsurf_WorkflowContentMatchesAsset(t *testing.T) {
	...
	got, err := os.ReadFile(filepath.Join(workspace, ".windsurf", "workflows", "opsx-propose.md"))
	if err != nil {
		t.Fatalf("ReadFile() error = %v", err)
	}

	want := assets.MustRead("windsurf/workflows/opsx-propose.md")
	if string(got) != want {
...
func TestInjectWindsurf_WorkflowsFoundFromSubdirectory(t *testing.T) {
	...
	// Simulate running jr-stack from a subdirectory inside that project.
	...
	expectedPath := filepath.Join(projectRoot, ".windsurf", "workflows", "opsx-propose.md")
	if _, err := os.Stat(expectedPath); err != nil {
		t.Fatalf("workflow not found at project root %q: %v", expectedPath, err)
	}

	// Must NOT be written inside the subdirectory.
	unexpectedPath := filepath.Join(subDir, ".windsurf", "workflows", "opsx-propose.md")
...
func TestInjectGeminiUsesAgentSpecificAsset(t *testing.T) {
	...
	if !strings.Contains(text, "~/.gemini/skills/") {
		t.Fatal("GEMINI.md missing ~/.gemini/skills/ path — agent-specific asset not used")
	}
...
func TestInjectCodexWritesSDDOrchestratorAndSkills(t *testing.T) {
	...
	text := string(content)
	if !strings.Contains(text, "OPSX Orchestrator") {
		t.Fatal("agents.md missing OPSX orchestrator content")
	}

	// Codex-specific asset must reference Codex skill paths.
	if !strings.Contains(text, "~/.codex/skills/") {
		t.Fatal("agents.md missing ~/.codex/skills/ path — agent-specific asset not used")
	}
...
func TestInjectCursorNoLegacySubAgentFiles(t *testing.T) {
	home := t.TempDir()
	...
	// Legacy sdd-* sub-agent files were removed in OPSX migration.
	// Verify none are written to disk.
	agentsDir := filepath.Join(home, ".cursor", "agents")
	legacyPhases := []string{"sdd-init", "sdd-explore", "sdd-propose", "sdd-spec", "sdd-design", "sdd-tasks", "sdd-apply", "sdd-verify", "sdd-archive"}

	for _, phase := range legacyPhases {
		agentPath := filepath.Join(agentsDir, phase+".md")
		if _, err := os.Stat(agentPath); err == nil {
			t.Fatalf("legacy agent file %s should NOT exist after OPSX migration", phase)
		}
	}
}
