...
func RunArgs(args []string, stdout io.Writer) error {
	...
	// manifests record which version of jr-stack created them.
	...
	if len(args) > 0 {
		...
		case "version", "--version", "-v":
			_, _ = fmt.Fprintf(stdout, "jr-stack %s\n", Version)
			return nil
		case "help", "--help", "-h":
	...
	// Self-update: check for a newer jr-stack release and apply it before
	...
	default:
		return fmt.Errorf("unknown command %q — run 'jr-stack help' for available commands", args[0])
	}
}
...
// runUpgrade handles the `jr-stack upgrade [--dry-run] [tool...]` command.
//
// This command:
//   - Checks for available updates for managed tools (jr-stack, engram, gga)
...
//   - Skips jr-stack itself when running as a dev build (version="dev")
