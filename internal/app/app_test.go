...
func TestListBackupsNewestFirst(t *testing.T) {
	home := t.TempDir()
	backupRoot := filepath.Join(home, ".jr-stack", "backups")
...
// TestRunArgsRestoreListIsDispatched verifies that `jr-stack restore --list`
...
// TestVersionBeforeSystemGuards verifies that `jr-stack version` returns the
...
func TestVersionBeforeSystemGuards(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "jr-stack") {
		t.Error("version output should contain 'jr-stack'")
	}
}
...
// an error whose message suggests running 'jr-stack help'.
func TestUnknownCommandSuggestsHelp(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "jr-stack help") {
		t.Error("unknown command error should suggest 'jr-stack help'")
	}
}
