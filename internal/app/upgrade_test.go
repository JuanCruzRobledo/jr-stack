...
// TestRunArgs_UpgradeDryRun verifies that `jr-stack upgrade --dry-run` runs without
...
// TestRunArgs_UpgradeNoArgs runs `jr-stack upgrade` without flags.
...
// TestRunArgs_UpgradeToolFilter verifies that `jr-stack upgrade engram` filters
...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	out := buf.String()
	// Output should only mention engram or no-upgrades, not jr-stack or gga.
	// This is a soft check since the tool may not be installed.
	if strings.Contains(out, "jr-stack") && !strings.Contains(out, "engram") {
		t.Errorf("filtering to engram should not show jr-stack in output; got: %s", out)
	}
}
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"jr-stack", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Gentleman-Programming/jr-stack/releases",
				},
			},
			wantContains:   []string{"jr-stack", "manual update required", "github.com", "[--]"},
			wantNotContain: []string{"[!!]", "FAILED"},
		},
		{
		...
		},
		{
			...
			wantContains:   []string{"engram", "[ok]", "jr-stack", "[--]", "gga", "1 succeeded", "2 skipped"},
			wantNotContain: []string{"FAILED", "[!!]"},
		},
	}
