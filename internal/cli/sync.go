...
//
// Discovery order:
//  1. Persisted state (~/.jr-stack/state.json) — written at install time.
...
func RenderSyncReport(result SyncResult) string {
	...
	if result.NoOp {
		fmt.Fprintln(&b, "jr-stack sync — no managed sync actions needed")
	...
	if result.DryRun {
		fmt.Fprintln(&b, "jr-stack sync — dry-run")
		fmt.Fprintf(&b, "Agents: %s\n", joinAgentIDs(result.Agents))
	...
	fmt.Fprintln(&b, "jr-stack sync — managed sync executed")
