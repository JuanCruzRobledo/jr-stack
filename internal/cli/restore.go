...
// RunRestore is the top-level entry point for `jr-stack restore [args]`.
...
func runRestoreWithHomeDir(args []string, restorer RestoreFunc, stdout io.Writer, stdin io.Reader, homeDir string) error {
	...
	if len(positional) == 0 {
		return fmt.Errorf("usage: jr-stack restore [--list | latest | <id>] [--yes]")
	}
...
// and the jr-stack version that created the backup when known.
...
func resolveRestoreTarget(target string, backups []backup.Manifest) (backup.Manifest, error) {
	...
	return backup.Manifest{}, fmt.Errorf("backup %q not found — use `jr-stack restore --list` to see available backups", target)
}
...
func backupRootDir(homeDir string) string {
	return homeDir + "/.jr-stack/backups"
}
