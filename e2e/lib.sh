...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/jr-stack inside
# the Docker container. If not found, fall back to $HOME/jr-stack or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./jr-stack in the current repo directory (freshly built local binary)
#   2. ~/jr-stack (explicit copy in home)
#   3. jr-stack on PATH (system-installed, e.g. Homebrew)
...
resolve_binary() {
    # Prefer the locally built binary (./jr-stack) produced by `go build ./cmd/jr-stack`.
...
# Checks that the jr-stack section marker appears exactly once (no duplicates).
